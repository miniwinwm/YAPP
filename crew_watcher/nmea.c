...
void process_rmc_message(unsigned char message_number)
{
	...
	if(next_message_field[0]=='A')
	{
		...
		latitude_degrees=(float)(unsigned int)latitude_dm;
		latitude_minutes=latitude_dm-latitude_degrees;
		...
		longitude_degrees=(float)(unsigned int)longitude_dm;
		longitude_minutes=longitude_dm-longitude_degrees;
		longitude_minutes*=100.0f;
	...
	{
		latitude_timestamp=0L;	
		longitude_timestamp=0L;	
	}		
}	
