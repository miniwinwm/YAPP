...
void serial_setup(void)
{
	...
	SPBRG1=51;					// baud Rate register to 4800 baud at 64MHz oscillator, section 16.3
...
{
	while(!TXSTA1bits.TRMT);	// wait until we're free to transmit, section 16.1.1.5
	TXREG1=c;					// transmit
}

// reads a character from the serial port without timeout
unsigned char getch(void)
{
	while(!PIR1bits.RC1IF);		// wait until data recieved, section 16.1.2.9
	return RCREG1;				// read and return
}
